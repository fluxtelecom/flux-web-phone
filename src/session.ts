...
export type WebPhoneSession = InviteClientContext &
    InviteServerContext & {
        ...
        canUseFluxMCallControl: typeof canUseFluxMCallControl;
...
export const patchIncomingSession = (session: WebPhoneSession): void => {
    ...
    session.canUseFluxMCallControl = canUseFluxMCallControl;
...
const parseRcHeader = (session: WebPhoneSession): any => {
    ...
    if (prcCallInfo && prcCallInfo.length) {
        ...
        if (rawCallInfo) {
            ...
            extend(session.fluxHeaders, parsed);
        }
    }
};
...
function canUseFluxMCallControl(this: WebPhoneSession): boolean {
    return !!this.fluxHeaders;
}

/*--------------------------------------------------------------------------------------------------------------------*/

function createSessionMessage(this: WebPhoneSession, options: FluxHeaders): string {
    if (!this.fluxHeaders) {
        return undefined;
    }
    extend(options, {
        sid: this.fluxHeaders.sid,
        request: this.fluxHeaders.request,
        from: this.fluxHeaders.to,
        to: this.fluxHeaders.from
    });
...
async function sendSessionMessage(this: WebPhoneSession, options): Promise<ClientContext> {
    ...
    return this.ua.sendMessage(this.fluxHeaders.from, this.createSessionMessage(options));
}
        ...
        session.sendRequest(C.INFO, {
            ...
            receiveResponse: (response: IncomingResponse) => {
                ...
                if (response.statusCode === 200) {
                    ...
                    const onInfo = (request: OutgoingRequest): void => {
                        ...
                        if (obj.response && obj.response.command === command.command) {
                            if (obj.response.result) {
                                ...
                                session.removeListener('FLUX_SIP_INFO', onInfo);
                    ...
                    session.on('FLUX_SIP_INFO' as any, onInfo);
                } else {
                    reject(
...
function receiveRequest(this: WebPhoneSession, request): any {
    switch (request.method) {
        ...
        case C.INFO:
            ...
            this.emit('FLUX_SIP_INFO', request);
